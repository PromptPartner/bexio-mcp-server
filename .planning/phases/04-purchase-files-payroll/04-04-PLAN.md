...
2. Create `src/tools/payroll/handlers.ts` with module detection:
   - Cache for module availability: `let payrollModuleAvailable: boolean | null = null;`
   - checkPayrollModule function: Probes on first call (list_employees with limit=1), caches result, returns boolean
   - payrollUnavailableError function: Returns McpError with friendly message per CONTEXT.md decision:
     "The Payroll module is not enabled in your Bexio account. To use employee and absence features, activate the Payroll module in your Bexio settings at https://office.bexio.com/settings/modules. The Payroll module is included with Pro+ plans or available as an add-on for Starter and Pro plans."
   - All handlers check module availability first, then proceed if available
   - Detection checks for 403/404 responses indicating module unavailable
...
IMPORTANT: Follow CONTEXT.md decision - friendly error messages, not technical/terse. Explain the situation helpfully.
  ...
  <verify>
    npm run build passes and `npm run build && node -e "const t = require('./dist/tools/index.js'); console.log('Payroll tools:', t.getAllToolDefinitions().filter(d => d.name.includes('employee') || d.name.includes('absence') || d.name.includes('payroll')).map(d => d.name))"` shows all 10 tools
  </verify>
  <done>
