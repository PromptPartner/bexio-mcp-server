...
- list_expenses, get_expense, create_expense, update_expense, delete_expense
  ...
  <verify>
    npm run build passes and `npm run build && node -e "const t = require('./dist/tools/index.js'); console.log('Purchase tools:', t.getAllToolDefinitions().filter(d => d.name.includes('bill') || d.name.includes('expense')).map(d => d.name))"` shows all 13 tools
  </verify>
  <done>
